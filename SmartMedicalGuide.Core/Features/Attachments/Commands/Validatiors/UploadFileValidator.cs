...
namespace SmartMedicalGuide.Core.Features.Attachments.Commands.Validators
{
    public class UploadFileValidator : AbstractValidator<UploadFileCommand>
    {
        ...
        public UploadFileValidator(UserManager<User> userManager)
        {
        ...
        public void ApplyValidationRules()
        {
            ...
            RuleFor(x => x.File)
                ...
                .Must(file => file != null && file.Length <= 10 * 1024 * 1024).WithMessage("File size must be less than 10MB")
                .Must(file => file != null && _allowedExtensions.Contains(Path.GetExtension(file.FileName).ToLowerInvariant()))
                .WithMessage($"File type not allowed. Allowed: {string.Join(", ", _allowedExtensions)}");
